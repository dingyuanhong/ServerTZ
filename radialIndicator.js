...
(function($, window, document) {
    ...
    function Indicator(container, indOption) {
        ...
        //add intial value 
        this.current_value = indOption.defaultValue || indOption.minValue || 0;
    ...
    Indicator.prototype = {
        ...
        init: function() {
            ...
            ctx.lineWidth = indOption.barBgWidth;
        ...
        value: function(val) {
            ...
            ctx.beginPath();
            if (val != 0) {
                var radius = indOption.radius;
                radius -= (indOption.barWidth - indOption.barBgWidth) / 2;
                ctx.arc(center, center, radius, -(quart), ((circ) * perVal / 100) - quart, false);
            }
            ctx.stroke();
            ctx.closePath();
            ctx.restore();
            ...
            if (indOption.displayNumber) {
                ...
                numberHeight = fontSize;
            }
            if (indOption.displayText) {
                var dispVal = indOption.text;
                var cFont = ctx.font.split(' '),
                    weight = indOption.textWeight,
                    fontSize = indOption.textSize || (dim / (this.maxLength - (Math.floor(this.maxLength * 1.4 / 4) - 1)));

                cFont = indOption.fontFamily || cFont[cFont.length - 1];

                ctx.fillStyle = indOption.textColor || curColor;
                ctx.font = weight + " " + fontSize + "px " + cFont;
                ctx.textAlign = "center";
                ctx.textBaseline = 'middle';
                if (dispVal != '') {
                    ctx.fillText(dispVal, center, center + numberHeight);
                }
            }
            return this;
        },
    ...
    radialIndicator.defaults = {
        ...
        defaultValue: 0 //define initial value of indicator
    };
    ...
    if ($) {
        $.fn.radialIndicator = function(options) {
            return this.each(function() {
                var obj = $.data(this, 'radialIndicator');
        ...
        $.fn.radialIndicatorObject = function(options) {
            if (this.length > 1) return null;
            var obj = this.data('radialIndicator');
            if (!obj) {
                var newPCObj = radialIndicator(this, options);
                this.data('radialIndicator', newPCObj);
                obj = newPCObj;
            }
            return obj;
        };
